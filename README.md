...
**Cleaning rules applied to M1 (spec section 4):**
...
- **Return-explosion filter** - drops bars where `|ret_1m| > 8 * MAD` AND
  ...
  **August 9, 2021 gold flash crash** (bars at ~01:57 UTC+2 with
  `spread_points approx. 9,000` and `ret_1m approx. -2.7%`) - the most dramatic data
...
The canonical schema applied to every output file (spec section 3.1):
